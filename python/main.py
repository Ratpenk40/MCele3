...
def main(argv):
  ...
  file_log_name_mex5_ratio_slow_fast = "logs/"+date_time+"/log_mex5_ratio_slow_fast.txt"
...
#  plt.ion()
  ...
  plk1_delay_start_time = 0.
  plk1_delay_end_time = 100.
  
  try:
    opts, args = getopt.getopt(argv,"hp:bfmds:",["particles=","bound","plk1", "threeD","slice","drawMovie","settings="])
  ...
  assert (v_mex5_fast!=0. and v_mex5_slow!=0. and v_plk1!=0. and plk1_to_mex5!=0.) , "something went wrong in reading the setting file"
  ...
  log.writeLine("plk1_detached_when_mex5_changes " + str(plk1_detached_when_mex5_changes))
  ...
  log.writeLine("log profile ratio slow/fast " + str(file_log_name_mex5_ratio_slow_fast))
  ...
  particle_family.SetSettings(initial_slow, initial_fast, k_fast_slow, k_slow_fast_low, k_slow_fast_high)
  ...
  if (plk1):
    particle_family_plk1.SetSettings(plk1_attach_to_mex_slow, plk1_attach_to_mex_fast, k_plk1_attach_to_free, plk1_detached_when_mex5_changes, plk1_delay_start_time, plk1_delay_end_time)
  ...
  for i in range(0, 1500):
    ...
    ratio, concid0, concid1, vel = plots.conc_calcCpp(X_list, Y_list,  Z_list, ID_list)
    ...
    if (plk1):
        ...
        particle_family_plk1.Move(v_plk1, v_mex5_slow, v_mex5_fast, 1, ratio, concid0, concid1, limits_particle, bound, i)
     ...
     #it is defined as follow: v standard, v_fast, v_slow, k_probability, dt, limits, nobound
        ...
        conc1, conc2, conc3, v_plk1_average = plots_plk1.conc_calcCpp(X_list_plk1, Y_list_plk1, Z_list_plk1, ID_list_plk1)
    ...
    if (i!=0 and i%10==0):
      ...
      if (plk1):
        ...
        plots_plk1.UpdateCpp(X_list_plk1, Y_list_plk1, Z_list_plk1, sliced, slice_depth)     

...
class Plot2D():
  ...
  def __init__(self, number, limits, limits_spheroid, v_mex5_slow, v_mex5_fast, path):
    ...
    os.mkdir(os.path.join(self.path, 'Graphs')) # creating the directory to store the graphs
...
#   ROOT Stuff   
  ...
  # Embryo plot with 3 subplots
    ...
    if self.counter % autosave == 0: 
      plt.imsave(os.path.join(self.path, f'Graphs/MEX-5-Embryo_t-{self.counter}.png'), 
    ...
    ax_id_ratio.set(title="Ratio, integrat. on Z", ylabel="Ratio MEX-5s/MEX-5f", xlabel="Embryo length (um)")
    
    ax_id_ratio2 = fig_id_ratio.add_subplot(3,2,2)
    ax_id_ratio2.plot(xbins_line, ratio2)
    ax_id_ratio2.set(title="Ratio, integrat. on Y and Z", ylabel="Ratio MEX-5s/MEX-5f", xlabel="Embryo length (um)")
